...
$allowedRoutes = [
    ...
    '/admin/prasarana/tanah/edit/([0-9]+)',
    // Prasarana - Gedung
...
if (!in_array($uri, $allowedRoutes) && !preg_match('/^\/admin\/prasarana\/tanah\/edit\/([0-9]+)$/', $uri)) {
...
switch ($uri) {
    ...
    case preg_match('/^\/admin\/prasarana\/tanah\/edit\/([0-9]+)$/', $uri, $matches) ? true : false:
        $id = $matches[1];
        auth();
        $admin = new TanahController();
        $admin->update($id);
