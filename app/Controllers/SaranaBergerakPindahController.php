...
class SaranaBergerakPindahController {
  private function renderView(string $view, $data = []) {
    extract($data);
    require_once __DIR__ . "/../Views/Pages/Pindah/SaranaBergerak/{$view}.php";
  }
  ...
  public function update($id) {
    ...
    if (!$sarana) {
      ...
      header('Location: /admin/sarana/bergerak/pindah');
      exit();
    }
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
      ...
      // Ambil status dari POST, jika tidak ada, gunakan status yang sudah ada di database, default 'Tersedia' jika tidak ada sama sekali
      $status = $_POST['status'] ?? $sarana['status'] ?? 'Tersedia';
      // Ambil detail peminjam dari data sarana yang ada, karena form pindah mungkin tidak mengubah ini
