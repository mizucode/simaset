...
class SaranaMebelairPindahController {
  ...
  public function create() {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
      ...
      // status dan peminjam fields, if applicable for create in "Pindah" context
  ...
  public function update($id) {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
      ...
      try {
        ...
        header('Location: /admin/sarana/mebelair/pindah?detail=' . $id); // Path redirect disesuaikan for Pindah context
        exit();
      } catch (PDOException $e) {
        $_SESSION['error'] = 'Error database: ' . $e->getMessage();
      }
  ...
  public function dokumen($id) // $id adalah ID Sarana Mebelair
  {
    ...
    if (!$mebelairData) {
      $_SESSION['error'] = 'Data Sarana Mebelair tidak ditemukan.';
      header('Location: /admin/sarana/mebelair/pindah');
      exit();
    }
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
      ...
      try {
        ...
        if ($success) {
          ...
          header('Location: /admin/sarana/mebelair/pindah?detail=' . $mebelairDataId);
          exit();
        }
      } catch (PDOException $e) {
        $_SESSION['error'] = 'Error database: ' . $e->getMessage();
  ...
  public function downloadDokumen($id) // $id adalah ID Dokumen Mebelair
  {
    ...
    if (!file_exists($filePath)) {
      ...
      header('Location: /admin/sarana/mebelair/pindah?detail=' . $dokumen['aset_mebelair_id']); // Kembali ke detail aset
      exit();
    }
  ...
  public function deleteDokumen() {
    if (isset($_GET['delete-dokumen']) && is_numeric($_GET['delete-dokumen'])) {
      ...
      if ($aset_mebelair_id) {
        header('Location: /admin/sarana/mebelair/pindah?detail=' . $aset_mebelair_id);
      } else {
        header('Location: /admin/sarana/mebelair/pindah'); // Fallback redirect
      }
      exit();
    }
  }
