...
class SaranaMebelairKondisiController {
  ...
  public function update($id) {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
      ...
      // Ambil status dan detail peminjam dari data sarana yang ada, karena form ini fokus pada kondisi
      $status = $sarana['status'] ?? 'Tersedia';
      ...
      try {
        ...
        header('Location: /admin/sarana/mebelair/kondisi'); // Redirect ke halaman daftar kondisi
        exit();
      } catch (PDOException $e) {
        $_SESSION['error'] = 'Error database: ' . $e->getMessage();
      }
