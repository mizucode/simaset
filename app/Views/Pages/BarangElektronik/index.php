...
<body class="hold-transition light-mode sidebar-mini layout-fixed layout-navbar-fixed layout-footer-fixed">
            ...
            <div class="card shadow-md">
              <div class="card-header bg-navy text-white d-flex justify-content-between align-items-center">
                ...
                <div class="ml-auto d-flex flex-column flex-sm-row align-items-sm-center">
                  <a href="/admin/sarana/elektronik/download-qr" class="btn btn-success btn-sm mb-2 mb-sm-0 mr-sm-2">
                  ...
                  <a href="/admin/sarana/elektronik/tambah" class="btn btn-warning btn-sm mb-2 mb-sm-0">
                        ...
                        <?php foreach ($saranaData as $barang) : ?>
                          <tr class="align-middle">
                            ...
                            <td class="text-center">
                              <input type="checkbox" class="row-checkbox" data-id="<?= htmlspecialchars($barang['id']); ?>">
                            </td>
                            ...
                            <td class="text-center">
                              <?php
                              $status = htmlspecialchars($barang['status'] ?? 'Tidak Diketahui');
                              $badgeClass = 'badge-secondary'; // Default badge
                              if ($status === 'Tersedia') $badgeClass = 'badge-success';
                              elseif ($status === 'Dipinjam') $badgeClass = 'badge-warning';
                              elseif ($status === 'Terpakai') $badgeClass = 'badge-info';
                              elseif ($status === 'Rusak Ringan' || $status === 'Rusak Berat') $badgeClass = 'badge-danger';
                              elseif ($status === 'Hilang') $badgeClass = 'badge-dark';
                              ?>
                              <span class="badge <?= $badgeClass; ?>"><?= $status; ?></span>
                            </td>
                            <td class="text-center">
                              <div class="d-flex justify-content-center gap-2">
                                <a href="/admin/sarana/elektronik/detail/<?= htmlspecialchars($barang['no_registrasi']); ?>" class="btn btn-info btn-sm">
  ...
  <script>
    $(function() {
      var table = $("#elektronikTable").DataTable({
        "responsive": true,
        "lengthChange": true,
        "autoWidth": false,
        "paging": true,
      ...
      $('#downloadSelectedQR').on('click', function() {
        ...
        var form = $('<form method="POST" action="/admin/sarana/elektronik/download-qr" target="_blank"></form>');
        $.each(selectedIds, function(index, id) {
          form.append($('<input type="hidden" name="selected_ids[]">').val(id));
        });
        $('body').append(form);
        form.submit().remove();
      });
