...
class SurveySaranaMebelair {
  public static function getAllData($conn) {
    $query = "SELECT * FROM survey_sarana_mebelair";
  ...
  public static function getAllDataInventaris($conn, $survey_id) {
    $query = "SELECT * FROM data_survey_mb WHERE survey_sarana_bergerak_id = :survey_id";
  ...
  public static function getById($conn, $id) {
    $query = "SELECT * FROM survey_sarana_mebelair WHERE id = :id LIMIT 1";
  ...
  public static function storeData(
    ...
    $query = "INSERT INTO survey_sarana_mebelair ($columns) VALUES ($placeholders)";
    $stmt = $conn->prepare($query);
  ...
  public static function updateData(
    ...
    $query = "UPDATE survey_sarana_mebelair SET
            penanggung_jawab = :penanggung_jawab,
            semester = :semester,
            tahun_akademik = :tahun_akademik,
            tanggal_pengecekan = :tanggal_pengecekan,
  ...
  public static function deleteData($conn, $id) {
    $query = "DELETE FROM survey_sarana_mebelair WHERE id = :id";
    $stmt = $conn->prepare($query);
    $stmt->bindParam(':id', $id);
    return $stmt->execute();
  }
  ...
  public static function storeDataInventaris(
    ...
    $query = "INSERT INTO data_survey_mb ($columns) VALUES ($placeholders)";
  ...
  public static function deleteDataInventaris($conn, $id) {
    $query = "DELETE FROM data_survey_mb WHERE id = :id";
    $stmt = $conn->prepare($query);
    $stmt->bindParam(':id', $id);
    return $stmt->execute();
  }
  ...
  public static function getInventarisItemById($conn, $id) {
    $query = "SELECT * FROM data_survey_mb WHERE id = :id LIMIT 1";
    try {
      $stmt = $conn->prepare($query);
      $stmt->bindParam(':id', $id, PDO::PARAM_INT);
      $stmt->execute();
